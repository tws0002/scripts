# pyside-uic -o //Art-1405260002/d/assets/scripts/maya_scripts/lib/qt_muster_ui.py //Art-1405260002/d/assets/scripts/maya_scripts/ui/qt_muster_submit.ui
import sys
...
import time
import qt_muster_ui_test as qt_muster_ui
...
def getPools():
    ...
    try:
        yyy = subprocess.Popen("mrtool -s " + server + " -port " + port + " -u \"admin\" -p \"\" -q p -H 0 -S 0 -pf parent", shell=True, stdout=subprocess.PIPE).communicate()[0]
        pool = yyy.split("\r\n")
        myset = set(pool)
        final = []
        for x in myset:
