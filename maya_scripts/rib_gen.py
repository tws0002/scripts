...
#---------------------------------------------------∏Ê©‰ª∂shapeÂÆöÁæ© Start---±Ê∏Ágroup‰∏≠ÈÅ∏shape------------------------------------------------------       
...
#-----------------------------------------define create folder Begin------------------------------------------------
    ...
    def createSelectFolder(self,singleGrp):   # singleGrp Ø‰ãÂèØ‰ª•Áî®‰æÜÂ≥ÂñÆ‰∏Ä∏ÂÑgroup transforme
...
#---------------------------------------------------export Cmd Start--------------------------------------------------------------------
    def getRibExportCmd(self,exportCmd): 
       ...
       # if str(int(self.ribGenDict['currentFrameCheck'])) == '1':
        if self.ribGenDict['currentFrameCheck'] == '1':
            ...
            # Ëº∏Âá∫ÄÁµÇÊ‰ª                               "rmanExportRIBCompression=0
            ...
            self.ribGenDict['finalGpuExportCmd'] = str("gpuCache -startTime "+"%s"%self.ribGenDict['currentFrameValue']+" -endTime "+"%s"%self.ribGenDict['currentFrameValue']+ " -optimize -optimizationThreshold 200000 -writeMaterials -dataFormat ogawa "+ "-directory "+"\"%s"%ribFileDir+"\"" + " -fileName " + "\" %s"%gpuCacheFileNameSingle+"\"")
                                                       #gpuCache    ãÂÇÈ                   ËÆäÊï∏ --------------------------ÁµêÊÇÈ-----------ËÆäÊï∏--------------------------------------------------------------------------------------------------------
    ...
    #        print ribExportSingalOption
           ...
           # cmd04='rmanExportEndFrame=%s;'%self.ribGenDict['endFrameValue']
            cmd03='rmanExportStartFrame=%s;'%startFrameReal #Ëº∏Âá∫ÑÊ‰ª§ÂàÊòØüÊ≠£ÑÊ º-1
            cmd04='rmanExportEndFrame=%s;'%endFrameReal #Ëº∏Âá∫ÑÊ‰ª§ÂàÊòØüÊ≠£ÑÊ º-1
            cmd05='rmanExportByFrame=1\"'
...
#---------------------------------------------------GPU RIB Cache Start--------------------------------------------------------
    ...
    def connectGpuRib(self,singleGrp):
        ...
        ribArchiveShapeCount = len(pm.ls(type='RenderManArchive'))      #Áµ±ËæÊÑRIB ArchiveÑÊï∏‰∏¶Âú®+1
        ...
        ribTransformName = pm.createNode( 'transform', n="%s"%ribArchiveNodetransformName+"RIBArchive%s"%ribArchiveShapeCountStr ,p='ribAssetGrp')  #¢ÁRIB Archive Ä‰∏äÂ±§group
      #  print ribTransformName
      #  print type(ribTransformName)
        ribArchiveShapeName = pm.createNode('RenderManArchive',n='%s'%ribArchiveNodetransformName+'_ribShape%s'%ribArchiveShapeCountStr) #¢ÁRIB Archive shape ‰∏¶Áî¢üÊÂ±Transform    
        gpuCacheShapeName = pm.createNode('gpuCache',n='%s'%ribArchiveNodetransformName+'_GpuCacheShape%s'%ribArchiveShapeCountStr) #¢ÁGPUCache shape ‰∏¶Áî¢üÊÂ±Transform    
        pm.parent(ribArchiveShapeName,ribTransformName)  #Â∞RIB Archive parent ÄÂ±¨Ê‰∏äÂ±§Group
        pm.parent(gpuCacheShapeName,ribTransformName)    #Â∞GpuCache parent ÄÂ±¨Ê‰∏äÂ±§Group
      ...
      #  print self.ribGenDict['motionBlurCheck']
        ...
        self.addRiAttribute(self)                         #ºÂè´ RIB Archive Ë∑GPUCache
...
#---------------------------------------------------Áµ¶‰ GPUCache Attribute Ê∏¨Ë©¶ begin--------------------------------------------------------  
...
#f.dumps(data)
        ...
        jsonRibTitle={ "Format" : "RIB Manifest",  #ribArchive ãÈ ≠Ë≥áÊÂª∫Á
        ...
        dtRibDict = {}    
        dictOrder =[] 
        jsObjTitle = json.dumps(jsonRibTitle,sort_keys=0,indent=4)[:-2] +",\n"      #ribArchive ãÈ ≠Ë≥áÊÂ≠òÂÖ•json buffer
        jsonName = path +'/'+ "RIBManifest.json"
        f = open(jsonName , 'w')   
        f.write(jsObjTitle)              #ribArchive ãÈ ≠Ë≥áÊÂØ´ÂÖ•json  
        
        addLineDriveFiles = "    "+"\""+"Driver-Files"+"\""+" : {"+" "  #ribArchive ãÈ ≠Ë≥áÊ,addLine,Â≠òÂÖ•json buffer
        f.write(addLineDriveFiles)      #ribArchive ãÈ ≠Ë≥áÊ,addLine ÂØ´ÂÖ•json
        while j <= endFrame:
            pm.currentTime(j,e=True)  # Ë∑≥Ëá≥‰∏ã‰
            
          #  print self.singleGrp
            bbSize = pm.xform(self.singleGrp,bb=True,q=True)  # ≤ÂbbBoxË≥áË
           ...
           # ribFileName = self.ribGenDict['ribArchiveFileName']
            jsonRibFrame={                                     #ribArchive each frame Ë≥áÊÂª∫Á
                       str(j):{"RIB-File" : ribFileName,
                        "Bounding-Box" : self.bBoxValue}
                       }
            jsObjFrame = json.dumps(jsonRibFrame,sort_keys=0,indent=8)[1:-2] +","   # ribArchive each frame Ë≥áÊÂ≠òÂÖ•json buffer
           # dictOrder.append(str(j))
            f.write(jsObjFrame)              # ribArchive each frame Ë≥áÊÂØ´ÂÖ•json   
            #print self.bBoxValue
            j = j + stepFrame  
           # print  pm.currentTime(q=True)

            #ãÂjsonÊ™îÊÔºåÂÁæ©ÊçÔÊ®°Â∫ÂØ´
...
#---------------------------------------------------Â∞árib archiveìÂêzipÊ™îÊ start-----------------------------------------------------------   
...
#------------------------------------------------run mel command ãÂ∑ËMel á‰ª§ start------------------------------
...
#------------------------------------------------run mel command ãÂ∑ËMel á‰ª§ end------------------------------
 

#---------------------------------------------------ãÂcache Ë≥áÊÂ§button Start-----------------------------------------------------------   
...
#------------------------------------------Generate RIB Archive Button Def Begin---------------------------------------
    def modpushButton_01(self):   
        ...
        if self.ribGenDict['subdivschemeCheck'] == '1':
            self.selectGrpList(self)                        #  Ä∏ÂÑGroup def
          #  print self.selectedTGrpList
            for self.singleGrp in self.selectedTGrpList:    #ãÂà•∏ÂGroup                              
            #   print self.singleGrp
                pm.select(self.singleGrp)
                self.createSelectFolder(self)               # define create folder def µÂª∫Ä∏ÂÑgrp folder
              ...
              #  print self.ribGenDict['finalGpuExportCmd']
               ...
               # print self.selectedGeoShape
                ...
                for self.singleShape in self.selectedGeoShape:                    #±group‰∏≠ÈÅ∏ñshape def
                #    print self.singleShape
                    self.addSubD(self)                                  #Â∞çÊ∏ÂÑÊ®°ãÂ SUBD
                self.gpuCacheMeshReduce (self)                    # ∑ËMel Command

                self.runMelCommandCreateCache(self)
                print self.singleGrp
                for self.singleShape in self.selectedGeoShape:                    #±group‰∏≠ÈÅ∏ñshape def
                 #   print self.singleShape
                    self.delSubD(self)                                  #Â∞çÊ∏ÂÑÊ®°ãÂ ™Èô§ SUBD               
                self.connectGpuRib(self)
                if self.ribGenDict['shutterTiming'] == "frameCenter":
                    self.batchRename(self)
