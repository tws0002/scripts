...
exp = "@GET(simpleslot/plan['id','2'].name)"
game = server.eval(exp)

filetype = "main"

exp = "@SOBJECT(simpleslot/game['name','" + game[0] + "'].simpleslot/3d.sthpw/snapshot.sthpw/file['file_name','not like','']['type','" + filetype + "'])"
temp = server.eval(exp)
items = temp

if len(temp) != 0: #if theres something in 3d, there's nothing in asset or shot, proceed to 2d
    exp = "@SOBJECT(simpleslot/game['name','" + game[0] + "'].simpleslot/package.sthpw/task.sthpw/snapshot.sthpw/file['file_name','not like','']['type','" + filetype + "'])"
    temp = server.eval(exp)
    items = items + temp

else: #if theres nothing in 3d, check assets
    exp = "@SOBJECT(simpleslot/game['name','" + game[0] + "'].simpleslot/assets.sthpw/snapshot.sthpw/file['file_name','not like','']['type','" + filetype + "'])"
    temp = server.eval(exp)
    items = items + temp
    if len(temp) != 0: #if theres somthing in assets, check shot
        exp = "@SOBJECT(simpleslot/game['name','" + game[0] + "'].simpleslot/shot.sthpw/snapshot.sthpw/file['file_name','not like','']['type','" + filetype + "'])"
        temp = server.eval(exp)
        items = items + temp        
        if len(temp) != 0: #if theres something in shot, check 2d
            exp = "@SOBJECT(simpleslot/game['name','" + game[0] + "'].simpleslot/package.sthpw/snapshot.sthpw/file['file_name','not like','']['type','" + filetype + "'])"
            temp = server.eval(exp)
            items = items + temp       
...
for name in name_list:
    ...
    for x in new_list:
		if x[6] == name:
			if x[8] == "sequence":
				if x[10] != "swf":
					images.append("<section><video controls loop preload='none' poster='/assets/%s' style='height=600px'><source src='/assets/%s' type='video/%s'></video>" % (x[11].decode("utf-8"),x[0].decode("utf-8"),x[10]))
				if x[10] == "swf":
					images.append("<section><object type='application/x-shockwave-flash' data='/assets/%s'></object>" % (x[0].decode("utf-8")))
			else:
				images.append("<section><img src = '/assets/%s' width = 800 '/>" % (x[0].decode("utf-8")))
